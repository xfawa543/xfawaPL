...
static int parse_block_and_emit(const char* file_base, const char* src, FILE* out, char declared[][64], int* decl_count) {
    ...
    while (*cur && brace_level > 0) {
        ...
        if (llen == 0) { cur = (line_end && *line_end) ? line_end + 1 : line_end; continue; }
                if (llen >= 6 && strncmp(line_start, "print(", 6) == 0) {
                    const char* s = line_start + 6; while (s < line_start + (int)llen && isspace((unsigned char)*s)) s++;
                    if (s >= line_start + (int)llen) { record_error(file_base, line_start, "print 缺少字符串"); print_error_context_with_lines(file_base, line_start, 2); cur = (line_end && *line_end) ? line_end + 1 : line_end; continue; }
                    const char* content_start = NULL; const char* content_end = NULL; int is_utf8_quote = 0;
                    if (*s == '"') {
                        content_start = s + 1;
                        const char* se = content_start;
                        while (se < le) {
                            if (*se == '\\' && (se + 1) < le) se += 2;
                            else if (*se == '"') break;
                            else se++;
                        }
                        if (se >= le) { record_error(file_base, content_start, "字符串未终止"); print_error_context_with_lines(file_base, content_start, 2); cur = (line_end && *line_end) ? line_end + 1 : line_end; continue; }
                        content_end = se;
                    } else if ((unsigned char)s[0] == 0xE2 && (unsigned char)s[1] == 0x80 && ((unsigned char)s[2] == 0x9C || (unsigned char)s[2] == 0x9D)) {
                        /* UTF-8 left/right double quotation marks: opening is E2 80 9C, closing E2 80 9D */
                        is_utf8_quote = 1;
                        content_start = s + 3;
                        const char* se = content_start;
                        while (se + 2 <= le) {
                            if ((unsigned char)se[0] == 0xE2 && (unsigned char)se[1] == 0x80 && (unsigned char)se[2] == 0x9D) break;
                            if (*se == '\\' && (se + 1) < le) se += 2; else se++;
                        }
                        if (se + 2 > le) { record_error(file_base, content_start, "字符串未终止(utf8引号)"); print_error_context_with_lines(file_base, content_start, 2); cur = (line_end && *line_end) ? line_end + 1 : line_end; continue; }
                        content_end = se;
                    } else {
                        record_error(file_base, s, "print 语法错误：期待字符串"); print_error_context_with_lines(file_base, s, 2); cur = (line_end && *line_end) ? line_end + 1 : line_end; continue;
                    }
                    fprintf(out, "    print_utf8(\"");
                    emit_escaped_string(out, content_start, (size_t)(content_end - content_start));
                    fprintf(out, "\");\n");
